...
public class ParkingDataChange {
	public static void indataChange(List<Parking> parking) {
		for(int i = 0; i < parking.size();i++) {
//			获取返回list集合中的每一项数据加以修改
			Parking flag = parking.get(i);
//			按指定格式设置开始时间（以小时分钟秒各两位显示）
...
//	车辆出场设置
	public static void outdataChange(Parking flag) {
//			读取当前时间
		Date date = new Date();
		...
		int moneyflag = 0;
		int hour = Integer.parseInt(begin.substring(0, 2));
		int minute = Integer.parseInt(begin.substring(3, 5));
		int second = Integer.parseInt(begin.substring(6, 8));
...
//	获取当前时间
	public static String nowDate() {
		...
		int nowminute = Integer.parseInt(df.format(date).substring(3,5));
		int nowsecond = Integer.parseInt(df.format(date).substring(6,8));
		return String.format("%1$02d", nowhour)+String.format("%1$02d", nowminute)+String.format("%1$02d", nowsecond);
	}
